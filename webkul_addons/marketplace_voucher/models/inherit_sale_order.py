...
class sale_order(models.Model):
	...
	def _add_voucher(self, wk_order_total , voucher_dict, so_id=False):
		...
		order_id = self.ids[0] if self.ids else so_id
		order_obj = self.browse(order_id)
		order_lines = False
		if voucher_obj.marketplace_seller_id:
			if order_lines := order_obj.order_line.filtered(
				lambda l: l.product_id.marketplace_seller_id
				and l.product_id.marketplace_seller_id.id
	...
	def write(self, vals):
		...
		applied_voucher = vals.get("applied_voucher") or self.applied_voucher
		if applied_voucher:
