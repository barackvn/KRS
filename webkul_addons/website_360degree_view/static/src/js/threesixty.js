...
{
  var page = $('#product_360_view').data('page');
if (page==='product')
{
...
var odoo_product = (0, _circlr2['default'])(el).scroll();
...
el.addEventListener('click', function (e) {
    ...
    document.getElementById("btn-play").innerHTML = '<span><i class="fa fa-play"></i></span>';
    $('.btn-play').removeClass('btn-danger');
} )

// Play Default
var default_play = parseInt($("#no_of_images").data("image-count"));
console.log(default_play);
odoo_product.play(default_play-1);

btnCycle.addEventListener('click', function (e) {
  toggleActive(e.target);
  odoo_product.cycle(isActive(e.target));
}, false);

btnReverse.addEventListener('click', function (e) {
  toggleActive(e.target);
  odoo_product.reverse(isActive(e.target));
}, false);
...
btnPlay.addEventListener('click', function (e) {
  if (document.getElementById("btn-play").innerHTML === '<span><i class="fa fa-play"></i></span>') {
    // alert(document.getElementById("btn-play").innerHTML);
    odoo_product.play();
    $('.btn-next').addClass('disabled');
    $('.btn-prev').addClass('disabled');
    $('.btn-next').attr('disabled', 'disabled');
    $('.btn-prev').attr('disabled', 'disabled');
    document.getElementById("btn-play").innerHTML = '<span><i class="fa fa-pause"></i></span>';
    $('.btn-play').addClass('btn-danger');
  }
  else if(document.getElementById("btn-play").innerHTML === '<span><i class="fa fa-pause"></i></span>'){
    odoo_product.stop();
    $('.btn-next').removeClass('disabled');
    $('.btn-prev').removeClass('disabled');
    $('.btn-next').removeAttr('disabled');
