...
class KanakAppointment(http.Controller):
    ...
    def appointment_member(self, partner_id=None, page=1, token=None, tz=None, **post):
        ...
        if browser_timezone := post.get('timezone'):
        ...
        if partner.min_date and partner.max_date:
            ...
            times = list(
                partner.get_range(
                    float(minutes_offset) / 60.0,
                    (1440 - time_offset) / 60.0,
                    (float(partner.minutes_slot) / 60),
                )
            )
            ...
            pager = request.website.pager(
                url=f'/member/appointment/{partner_id}',
            ...
            for i in range(self._total_days):
                ...
                if partner[dayname.lower()]:
                    ...
                    new_start_time = self.value_to_string_html(partner[f'{dayname.lower()}_from'])
                    new_end_time = self.value_to_string_html(partner[f'{dayname.lower()}_to'])
            ...
            max_range = sorted(
                {item for sublist in partner_available_time for item in sublist}
            )
            values = {
                'partner': partner,
            ...
            if token:
                ...
                pager = request.website.pager(
                    url=f'/appointment/reschedule/{partner_id}?token={token}',
                    total=self._total_days,
                    page=page,
                    step=self._days_per_page,
                )
                values |= {
                    ...
                    'set_timezone': appointment.partner_id.tz,
                    'pager': pager,
                }
    ...
    def book_member(self, partner_id=None, **post):
        if not post:
            ...
            return request.redirect(f'/member/appointment/{partner_id.id}')
        partner = request.env['res.partner'].sudo().browse(partner_id)
        ...
        post['req_partner'] = (
            None if request.env.user._is_public() else request.env.user.partner_id
        )
    ...
    def booking_confirm(self, partner_id=None, **post):
        ...
        if not partner:
            partner = Partner.sudo().create(
                {
                    'name': f"{post.get('first_name')} {post.get('last_name') and post.get('last_name') or ''}",
        ...
        event = {
            'name': f"{post.get('first_name')}-{fields.Datetime.to_string(date)}",
            ...
            'end_date_time': fields.Datetime.to_string(
                (utc_date + timedelta(minutes=int(appointment_with.minutes_slot)))
            ),
            'meet_flag': True,
            'type': 'appointment',
    ...
    def reschedule_confirm(self, partner_id=None, access_token=None, **post):
        ...
        appointment.event_id.sudo().write(
            {
                'name': f'{appointment.partner_id.name}-{fields.Datetime.to_string(date)}',
                'app_date': fields.Datetime.to_string(utc_date),
                'app_time': str(utc_date.strftime('%H:%M')),
                'date_appointment': date_appointment,
                'start': fields.Datetime.to_string(utc_date),
                'stop': fields.Datetime.to_string(
                    (utc_date + timedelta(minutes=int(partner.minutes_slot)))
                ),
                'start_datetime': fields.Datetime.to_string(utc_date),
                'stop_datetime': fields.Datetime.to_string(
                    (utc_date + timedelta(minutes=int(partner.minutes_slot)))
    ...
    def booking_cancel(self, partner_id=None, token=None, **post):
        ...
        post |= {
            'appointment': appointment,
            'partner': partner,
            'date_appointment': appointment.event_id.date_appointment,
        }
