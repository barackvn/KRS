...
from odoo.addons.web.controllers.main import module_installed as inherit_module_installed
from odoo.addons.web.controllers import main as inherit_module_installed_obj
...
def module_installed(environment):
    sorted_modules = inherit_module_installed(environment)
    if 'website_advertisement_manager' in sorted_modules:
        sorted_modules.remove('website_advertisement_manager')
    if 'marketplace_advertisement_manager' in sorted_modules:
        sorted_modules.remove('marketplace_advertisement_manager')
    return sorted_modules

inherit_module_installed_obj.module_installed = module_installed
...
class AdvertiseWebsiteSale(WebsiteSale):

    def _get_search_domain(self, search, category, attrib_values, search_in_description=True):
        
        res = super(AdvertiseWebsiteSale, self)._get_search_domain(search, category, attrib_values)
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        res = super(AdvertiseWebsiteSale, self).cart_update(product_id, add_qty, set_qty, **kw,)
        return res
