...
class WebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        product_tags_limit = IrDefault.get(
            'website.product.tags.setting', 'product_tags_limit')
        response.qcontext['product_tags'] = request.website._get_active_tags(
            limit=product_tags_limit)
        wk_product_tag_ids = request.session.get('wk_product_tag_ids')
        if wk_product_tag_ids:
            wk_product_tag_ids = wk_product_tag_ids[2]
            response.qcontext['wk_tags_list'] = Tag.sudo().browse(
                wk_product_tag_ids)
        return response

    @http.route(['/product/tags/remove/<tag_id>'], type='http', auth="public",  website=True)
    def product_tags_remove(self, tag_id):
        if request.session.get('wk_product_tag_ids') and int(tag_id) in request.session.get('wk_product_tag_ids')[2]:
