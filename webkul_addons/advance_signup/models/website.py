...
class Website(models.Model):
    ...
    def get_active_website_setting(self):
        if website_id := self.env['website'].get_current_website():
            return (
                self.env['adv.signup.settings']
                .sudo()
                .search(
        ...
        if objs:
            for rec in objs:
                ...
                if field_type.name.startswith('property_'):
                    f_values[f'{field_type.relation},{str(rec.id)}'] = rec.name
                else:
            ...
            for field in signup_obj.signup_field_ids:
                ...
                field_dict = {
                    f_name: {
                        ...
                        or field.field.field_description
                        or '',
    ...
    def is_login_signup_page(self):
        ...
        return (
            path.find("/web/login") == 0
            or path.find("/web/signup") == 0
            or path.find("/web/reset_password") == 0
        )

    def is_signup_page(self):
        return request.httprequest.full_path.find('/web/signup') == 0
