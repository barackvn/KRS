...
class CrmLeadInherit(models.Model):
    ...
    def action_accept_lead(self):
        for record in self:
            if record.partner_id.seller:
                url_approve1 = f'{record.get_base_url()}/approve/{str(record.id)}'
                url_reject1 = f'{record.get_base_url()}/reject/{str(record.id)}'
    ...
    def scheduler_seller_buyer_crm_loss_won(self):
        ...
        for lead in buyer_lead_ids:
            if lead.partner_id:
                if order_id := self.env['sale.order'].search(
        ...
        for member in seller_lead_ids:
            if member:
                if mermbership := self.env['seller.membership'].search(
                    [
    ...
    def send_to_approve(self):
        if self.partner_id.name and self.partner_id.email:
            mail_values = (
                ...
                | {
                    'subject': f'{self.partner_id.name}Quotation Approval',
